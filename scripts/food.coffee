...
radius = process.env.HUBOT_YELP_SEARCH_RADIUS or 20000
...
search_yelp = (msg, query, location, radius, random, callback) ->
  # Perform the search
  #msg.send("Looking for #{query} around #{location}...")
  yelp.search category_filter: "restaurants", term: query, radius_filter: radius, sort: sort, limit: 2, location: location, (error, data) ->
    if error != null
    ...
    if random
      ...
      msg.message.user['last_yelp_restaurant_place'] = location
...
lunchMe = (msg, query, random = true) ->
  ...
  if (typeof location == "undefined" || location == "")
    ...
    msg.waitResponse regex, (msg)->
      ...
      search_yelp msg, query, location, radius, random, (business)->
  ...
  search_yelp msg, query, location, radius, random, (business)->
    msg.send("How about " + business.name + "? " + business.url)
    msg.send("they are located at " + business.location.display_address)
    msg.send("you can call them " + business.display_phone)
